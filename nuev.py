...
def cruzaop(mejores):
    ...
    try:
        results = pool.map(uno, [ m for m in mejores ])
    except:
        print("feil")
...
def sort(l):
    return l if len(l)<2 else sort([li for li in l[1:] if li[1]<l[0][1]])+[l[0]]+sort([li for li in l[1:] if li[1]>=l[0][1]])
...
for i in range(15):
    ...
    mejores = selecciona(Va,100)+reserva
